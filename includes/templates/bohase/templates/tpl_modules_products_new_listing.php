...
<?php
  ...
  if ($products_new_split->number_of_rows > 0) {
	...
	$products_description_full = zen_trunc_string(zen_clean_html(stripslashes(zen_get_products_description($products_new->fields['products_id'], $_SESSION['languages_id']))), PRODUCT_LIST_DESCRIPTION); //To Display Product Desc 
	
	$products_description_list = ltrim(substr($products_description_full, 0, 250) . ''); //Trims and Limits the desc
	
	$moreinfo = '<a class="more_info_text" href="' . zen_href_link(zen_get_info_page($products_new->fields['products_id']), 'cPath=' . zen_get_generated_category_path_rev($products_new->fields['master_categories_id']) . '&products_id=' . $products_new->fields['products_id']) . '">'.MORE_INFO_TEXT.'</a>';
...
//            $how_many++;
      ...
      if (PRODUCT_NEW_LIST_DESCRIPTION > '0') {
        ...
        $display_products_description = '';
			...
			else if(mb_featured_product($products_new->fields['products_id'])==1){
				$ribbon="<div class='tag tag-orange' title=''><div class='text'>HOT</div></div>";
			}
			else {
			}
			
			$pid = $products_new->fields['products_id'];
			...
			/*Wishlist Links*/
			if (UN_MODULE_WISHLISTS_ENABLED) { $wishlist_link= '<a class="lnk" href="' . zen_href_link(UN_FILENAME_WISHLIST, 'products_id=' . $products_new->fields['products_id'] . '&action=wishlist_add_product') . '"><i class="fa fa-heart"></i>Add to Wishlist</a>';}else{ $wishlist_link='';}
	
			$compare_link='<a class="lnk" href="javascript: compareNew('.$products_new->fields['products_id'].', \'add\')"><i class="fa fa-exchange"></i>Add to Compare</a>';
    	...
    	<div data-filter="all products" data-name="<?php echo $products_name; ?>" data-discount="0" data-price="110" class="item product-item">
            ...
            <div class="product grid-view">
                ...
                <div class="product-info">
                    ...
                    <?php echo mb_product_reviews($products_new->fields['products_id']);?>
                    <div class="price">
