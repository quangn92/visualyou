      ...
      if (PRODUCT_FEATURED_LIST_DESCRIPTION > '0') {
        ...
        $display_products_description = '';
			...
			if (UN_MODULE_WISHLISTS_ENABLED) { 
				$gridview_wishlist_link= zen_href_link(UN_FILENAME_WISHLIST, 'products_id=' . $featured_products->fields['products_id'] . '&action=wishlist_add_product');
			}else{ $gridview_wishlist_link=''; }
	
			$gridview_compare_link='javascript: compareNew('.$featured_products->fields['products_id'].', \'add\')';
			
			/*Add to Cart Button*/
			$gridview_buy_now = zen_get_buy_now_button($featured_products->fields['products_id'],'');
			if($gridview_buy_now!=NULL){
				$gridview_buy_now = '<a title="Sold Out" class="detailbutton-wrapper add-to-cart" href="' . zen_href_link(zen_get_info_page($featured_products->fields['products_id']), 'cPath=' . zen_get_generated_category_path_rev($featured_products->fields['master_categories_id']) . '&products_id=' . $featured_products->fields['products_id']) . '"><i class="fa fa-ban fa-lg"></i></a>';
			}
			elseif($attribute_product == $pid) {
				$gridview_buy_now = '<a title="Add to Cart" class="detailbutton-wrapper add-to-cart" href="' . zen_href_link($_GET['main_page'], zen_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $featured_products->fields['products_id']) . '"><i class="fa fa-shopping-cart fa-lg"></i></a>'; 
			}
			else {				
				$gridview_buy_now = '<a title="Add to Cart" class="detailbutton-wrapper add-to-cart" href="' . zen_href_link($_GET['main_page'], zen_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $featured_products->fields['products_id']) . '"><i class="fa fa-shopping-cart fa-lg"></i></a>'; 
			}
