...
if ($num_products_count > 0) {
  ...
  while (!$specials_index->EOF) {
	...
	if (UN_MODULE_WISHLISTS_ENABLED) { 
		$wishlist_link= zen_href_link(UN_FILENAME_WISHLIST, 'products_id=' . $specials_index->fields['products_id'] . '&action=wishlist_add_product');
	}
	else{ $wishlist_link=''; }
	
	$compare_link='javascript: compareNew('.$specials_index->fields['products_id'].', \'add\')';
	
	$buy_now = zen_get_buy_now_button($specials_index->fields['products_id'],'');
	if($buy_now!=NULL){
		$buy_now = '<a title="Sold Out" class="detailbutton-wrapper add-to-cart" href="' . zen_href_link(zen_get_info_page($specials_index->fields['products_id']), 'cPath=' . $productsInCategory[$specials_index->fields['products_id']] . '&products_id=' . $specials_index->fields['products_id']) . '"><i class="fa fa-ban fa-lg"></i></a>';
	}
	elseif($attribute_product == $sid) {
		$buy_now = '<a title="Add to Cart" class="detailbutton-wrapper add-to-cart" href="' . zen_href_link($_GET['main_page'], zen_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $specials_index->fields['products_id']) . '"><i class="fa fa-shopping-cart fa-lg"></i></a>'; 
	}
	else {
		$buy_now = '<a title="Add to Cart" class="detailbutton-wrapper add-to-cart" href="' . zen_href_link($_GET['main_page'], zen_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $specials_index->fields['products_id']) . '"><i class="fa fa-shopping-cart fa-lg"></i></a>'; 
	}
    ...
    $list_box_contents[$row][$col] = array('params' => 'class="item centerBoxContentsSpecials product-item back"' . ' ', 'text' => (($specials_index->fields['products_image'] == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0) ? '' : 
	'<div class="product">
  ...
  if ($specials_index->RecordCount() > 0) {
    $title = '<h3 class="section-title">'. sprintf(TABLE_HEADING_SPECIALS_INDEX, strftime('%B')) .'</h3>';
